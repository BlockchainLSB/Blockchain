...
function generateWait() {
  echo "*** Wait for 7 minutes to make sure the certificates become active ***"
  sleep 7m
}
...
if [ "${MODE}" == "up" -a "${ORG}" == "" ]; then
  ...
  createChannelAndJoin ${ORG1} common
  createChannelAndJoin ${ORG1} common
  createChannelAndJoin ${ORG1} "${ORG1}-${ORG2}"
  createChannelAndJoin ${ORG1} "${ORG1}-${ORG3}"
  joinChannel ${ORG2} common
  joinChannel ${ORG2} "${ORG1}-${ORG2}"

  createChannelAndJoin ${ORG2} "${ORG2}-${ORG3}"
  joinChannel ${ORG3} common
  joinChannel ${ORG3} "${ORG1}-${ORG3}"
